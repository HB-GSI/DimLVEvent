...
#define DIM_VERSION_NUMBER 1587282432
...
//handles and routine re-definition for dynamic loading of DIM dll. This is required since the dimStd.dll
//is not loaded implecitely, but dynamically during run-time
void      *libDimHandle;                  //handle to shared library of DIM
#define dic_info_service           (*dicp_info_service)
#define dic_info_service_stamped   (*dicp_info_service_stamped)
#define dic_release_service        (*dicp_release_service)
#define dic_add_error_handler      (*dicp_add_error_handler)
#define dic_dim_stop               (*dicp_dim_stop)
#define dic_get_server             (*dicp_get_server)
#define dic_get_format             (*dicp_get_format)
#define dic_get_timestamp          (*dicp_get_timestamp)
#define dic_get_quality            (*dicp_get_quality)
#define dic_cmnd_service           (*dicp_cmnd_service)
#define dis_remove_service         (*disp_remove_service)
#define dis_add_exit_handler       (*disp_add_exit_handler)
#define dis_add_error_handler      (*disp_add_error_handler)
#define dis_stop_serving           (*disp_stop_serving)
#define dis_get_client             (*disp_get_client)
#define dis_add_service            (*disp_add_service)
#define dis_update_service         (*disp_update_service)
#define dis_set_quality            (*disp_set_quality)
#define dis_set_timestamp          (*disp_set_timestamp)
#define dis_start_serving          (*disp_start_serving)
#define dis_add_cmnd               (*disp_add_cmnd)
#define dim_set_dns_node           (*dimp_set_dns_node)
#define dim_get_dns_node           (*dimp_get_dns_node)
unsigned  (*dicp_info_service)(char *, int, int, int *, int, void *, long, int*, int);
unsigned  dic_info_service_stamped(char *, int, int, int *, int, void *, long, int *, int); 
void      dic_release_service(unsigned);
void      dic_add_error_handler(void *);
void      dic_dim_stop(void);
int       dic_get_server(char *);
char *    dic_get_format(unsigned);
int       dic_get_timestamp(unsigned, int *, int *);
int       dic_get_quality(unsigned);
int       dic_cmnd_service(char *, int *, int);
...
unsigned  dis_add_service(char *, char *, int *, int, void *, long);
...
char						noLink[MAXLEN];					//default data of any service
...
//still "global", pointers for handling access to DIM library
...
HINSTANCE					myDimStdLib;					//ref to DIM library
#endif
#ifdef LINUX
time_t						actTime;
#endif
...
#endif
	sprintf(my_message, "%.19s, server %s, PID %d: ", timeline, dimServerName, getpid());
...
//-----------------------------------------------------------------------------------
// author  : Dietrich Beck, GSI-Darmstadt
// purpose : dynamicLibraryLoad: loads the DIM library dynamically
// returns : error - 1: no error; 0 : error
...
#ifdef LINUX
	myErrMessage = dlerror(); //clear error message
	libDimHandle = dlopen("libmydim.so", RTLD_NOW);
	if (libDimHandle)
	{
		dicp_info_service           = dlsym(libDimHandle, "dic_info_service_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_info_service_stamped   = dlsym(libDimHandle, "dic_info_service_stamped_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_release_service        = dlsym(libDimHandle, "dic_release_service_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_add_error_handler      = dlsym(libDimHandle, "dic_add_error_handler");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_dim_stop               = dlsym(libDimHandle, "dic_dim_stop");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_get_server             = dlsym(libDimHandle, "dic_get_server");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_get_format             = dlsym(libDimHandle, "dic_get_format_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_get_timestamp          = dlsym(libDimHandle, "dic_get_timestamp_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_get_quality            = dlsym(libDimHandle, "dic_get_quality_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dicp_cmnd_service           = dlsym(libDimHandle, "dic_cmnd_service_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_remove_service         = dlsym(libDimHandle, "dis_remove_service_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_add_exit_handler       = dlsym(libDimHandle, "dis_add_exit_handler_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_add_error_handler      = dlsym(libDimHandle, "dis_add_error_handler");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_stop_serving           = dlsym(libDimHandle, "dis_stop_serving_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_get_client             = dlsym(libDimHandle, "dis_get_client_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_add_service            = dlsym(libDimHandle, "dis_add_service_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_update_service         = dlsym(libDimHandle, "dis_update_service_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_set_quality            = dlsym(libDimHandle, "dis_set_quality_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_set_timestamp          = dlsym(libDimHandle, "dis_set_timestamp_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_start_serving          = dlsym(libDimHandle, "dis_start_serving_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		disp_add_cmnd               = dlsym(libDimHandle, "dis_add_cmnd_");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dimp_set_dns_node           = dlsym(libDimHandle, "dim_set_dns_node");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
		dimp_get_dns_node           = dlsym(libDimHandle, "dim_get_dns_node");
		if ((myErrMessage = dlerror()) != NULL) {dw_printf(myErrMessage); myError = 1;}
	} //if libDimHandle
	else 
	{
...
#endif
	if (myError) dw_printf("DimWrapper - FATAL!!!!!!!!!!!!!!!!!! \n");
...
//-----------------------------------------------------------------------------------
// author  : Dietrich Beck, GSI-Darmstadt
// purpose : dynamicLibraryClose: closes the DIM library, that was opened dynamically
// returns : void
void dynamicLibraryClose()
{
...
void initMyLib()
{
	...
	//disable DLL_PROCESS_ATTACH and DLL_PROCESS_DETACH calls of myDimStd lib
	//myDimStdLib = LoadLibrary(TEXT("myDimStd"));
	//DisableThreadLibraryCalls(myDimStdLib);
	//FreeLibrary(myDimStdLib);
#endif

	dw_printf("DimWrapper: dynloading\n");
	dynamicLibraryLoad();
	dw_printf("DimWrapper: dynload done\n");
	...
	_ftime(&stopTime);
	timePerIteration = (stopTime.time  - startTime.time)    * 1000000;
	timePerIteration = (stopTime.millitm - startTime.millitm) * 1000 - timePerIteration;
...
void deinitMyLib()
{
	...
	dic_dim_stop(); //self-built patch to dim 17.11. Will be included in the next dim release and this routine
	                //will be called dic_stop(). So this function needs to be renamed when building this
	                //library with a newer version of dim.
	...
	dw_printf("DimWrapper: dynunloading\n");
	for (i=0;i<100;i++) dynamicLibraryClose();
	dw_printf("DimWrapper: dynunload done\n");
	sleep(1);
...
int *size;              //length of available data
{
	...
	if ((securityEnabled) && (allowedNodes != NULL))
	{
		...
		if (strlen(myNode) > 1) //don't care about undefined node names, strlen <1 is probably a "NOLINK"
		{
			...
			{
				sprintf(myMessage, "PID %d, server %s: illegal service received from node %s\n", getpid(), dimServerName, myNode);
				dw_printf(myMessage);
				executingCallback = 0;
				return;
	...
	{
		sprintf(myMessage, "PID %d, server %s: DIMWrapper: receivedCB can't get rid of data!\n", getpid(), dimServerName);
		dw_printf(myMessage);
		executingCallback = 0;
		return;
	}
	...
	{
		sprintf(myMessage, "PID %d, server %s: DIMWrapper: receivedCB, data to large!\n", getpid(), dimServerName);
		dw_printf(myMessage); 
		executingCallback = 0;
		return;
	}
								    ...
								    )
{
	...
	if (safeMode && (size > maxLocalBuffSize)) return 0;
